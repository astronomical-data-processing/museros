...
//__global float d_Kernel_rows[KERNEL_W];
//__global float d_Kernel_columns[KERNEL_W];
...
__kernel void moving_average_1d_GPU(__global int *in,__global int *out, int M, int N, int RADIUS) {
...
__kernel void convolutionRowGPU(
    __global float *d_Kernel_rows,
    __global float *d_Kernel_columns,
    ...
    //Data cache
    __local float data[KERNEL_RADIUS + ROW_TILE_W + KERNEL_RADIUS];

    //Current tile and apron limits, relative to row start
    int blockIdxx=(get_global_id(0)-get_local_id(0))/get_local_size(0);
    const int         tileStart = IMUL(blockIdxx, ROW_TILE_W);
    ...
    int blockIdxy=(get_global_id(1)-get_local_id(1))/get_local_size(1);
    const int          rowStart = IMUL(blockIdxy, dataW);
    ...
    const int loadPos = apronStartAligned + threadIdxx;
    ...
    //int threadIdxx = get_local_id(0);
    const int writePos = tileStart + threadIdxx;
...
__kernel void convolutionColumnGPU(
    ...
    //Data cache
    __local float data[COLUMN_TILE_W *
    (KERNEL_RADIUS + COLUMN_TILE_H + KERNEL_RADIUS)];

    //Current tile and apron limits, in rows
    int blockIdxy=(get_global_id(1)-get_local_id(1))/get_local_size(1);
    const int         tileStart = IMUL(blockIdxy, COLUMN_TILE_H);
    ...
    int blockIdxx=(get_global_id(0)-get_local_id(0))/get_local_size(0);
    int threadIdxx = get_local_id(0);
    const int       columnStart = IMUL(blockIdxx, COLUMN_TILE_W) + threadIdxx;

    //Shared and global memory indices for current column
    //int threadIdxx=get_local_id(0);
    int threadIdxy=get_local_id(1);
    int smemPos = IMUL(threadIdxy, COLUMN_TILE_W) + threadIdxx;
    int gmemPos = IMUL(apronStart + threadIdxy, dataW) + columnStart;
    //Cycle through the entire data cache
    //Load global memory values, if indices are within the image borders,
    //or initialize with zero otherwise
    int blockDimy=get_local_size(1);
    for(int y = apronStart + threadIdxy; y <= apronEnd; y += blockDimy){
        data[smemPos] =
    ...
    barrier(CLK_LOCAL_MEM_FENCE);
    //Shared and global memory indices for current column
    smemPos = IMUL(threadIdxy + KERNEL_RADIUS, COLUMN_TILE_W) + threadIdxx;
    gmemPos = IMUL(tileStart + threadIdxy , dataW) + columnStart;
    //Cycle through the tile body, clamped by image borders
    //Calculate and output the results
    for(int y = tileStart + threadIdxy; y <= tileEndClamped; y += blockDimy){
        float sum = 0;
'''
originalLoop = '''
        for(int k = -KERNEL_RADIUS; k <= KERNEL_RADIUS; k++)
...
prg1 = cl.Program(ctx,code).build()
convolutionRowGPU = prg1.convolutionRowGPU
convolutionColumnGPU = prg1.convolutionColumnGPU
meanaverageGPU = prg1.moving_average_1d_GPU

#d_Kernel_rows = prg1.get_global('d_Kernel_rows')[0]
#d_Kernel_columns = prg1.get_global('d_Kernel_columns')[0]
...
def gaussian_kernel(width = KERNEL_W, sigma = 64.0):
    ...
    radius = (width - 1)/2.0
    x = np.ones(width) #numpy.linspace(-radius,  radius,  width)
    x = np.float32(x)   #x = numpy.float32(x)
...
def convolution_cuda(sourceImage,  filterx,  filtery):
    ...
    #destImage_gpu = cl_array.zeros_like(destImage)
    destImage_gpu = cl_array.to_device(queue,destImage)
    #sourceImage_gpu = cl_array.to_device(queue,sourceImage)
    intermediateImage_gpu = cl_array.to_device(queue,sourceImage)
    sourceImage_gpu = cl_array.to_device(queue,sourceImage)
    d_Kernel_rows = cl_array.to_device(queue,filterx)
    d_Kernel_columns = cl_array.to_device(queue,filtery)
    ...
    convolutionRowGPU(queue,intermediateImage_gpu.shape,None,d_Kernel_rows,d_Kernel_columns,intermediateImage_gpu.data,sourceImage_gpu.data,DATA_W,DATA_H)
    convolutionColumnGPU(queue,intermediateImage_gpu.shape,None,d_Kernel_rows,d_Kernel_columns,destImage_gpu.data,intermediateImage_gpu.data,DATA_W,DATA_H,np.int32(COLUMN_TILE_W * threadBlockColumns[1]),  np.int32(DATA_W * threadBlockColumns[1]))
    # Pull the data back from the GPU.
    #cuda.memcpy_dtoh(destImage,  destImage_gpu)
    destImage = destImage_gpu.get()
    return destImage
...
def mean_average_1d(in_image, out_image, height, width, radius):
    ...
    globalsize2D=(width,height)
    #meanaverageGPU(in_image, out_image, np.int32(height), np.int32(width), np.int32(radius), grid=gridsize2D,  block=blocksize2D)
    meanaverageGPU(queue,globalsize2D,None,in_image, out_image, np.int32(height), np.int32(width), np.int32(radius))
    return out_image

def test_convolution_cuda():
    # Test the convolution kernel.
    # Generate or load a test image
    original = np.random.rand(1, 2048) * 255
...
class Normal(object):
    ...
    def update(self, mu, sigma):
        ...
        if self.dim == 1:
            ...
            #m = 10^-6
            #self.A = la.inv(self.E+np.eye(self.E.shape[1])*m) # precision matrix
