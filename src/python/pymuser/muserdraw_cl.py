...
class MuserDraw:
    ...
    def draw_one(self,filename, title, data, xleft=0, xright=0, ybottom=0, ytop=0, radius=0, axis=True, axistype = 0):
        if axistype ==0:
            vra = [np.percentile(data, 1), np.percentile(data, 100)]

            fig, axs = plt.subplots()  # 1, 2, sharex=True, sharey=True, figsize=(12.2, 6));
            plt.subplots_adjust(wspace=0)
            # im = axs.imshow(np.flipud(data), vmin=vra[0], vmax=vra[1], cmap=cm.jet, origin='lower')
            im = axs.imshow(data, vmin=vra[0], vmax=vra[1], cmap=cm.jet, origin='upper')
            axs.set_title(title)
            ...
            vra = [np.percentile(data, 0), np.percentile(data, 100)]
            ...
            if not axis:
                ...
                gci=plt.imshow(data, extent=extent, origin='upper',cmap=cmap, vmin=vra[0], vmax=vra[1])
                plt.axis('off')
