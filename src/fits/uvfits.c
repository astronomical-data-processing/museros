...
int writeUVFITS(char *filename, uvdata *data) {
  ...
  fits_update_key(fptr,TSTRING, "TELESCOP", "LWA-OVRO" , NULL, &status);
  fits_update_key(fptr,TSTRING, "INSTRUME", "LEDA-512" , NULL, &status);
  temp=2000.0;
  ...
  fits_write_comment(fptr,"written by the UV FITS writer of CSRH.",&status);
  ...
  status = writeFQbintable(fptr);
  if (status !=0) {
    fprintf(stderr,"ERROR: writeFQ failed with status %d\n",status);
  }
  status = writeAntennaData(fptr, data);
  if (status !=0) {
    fprintf(stderr,"ERROR: writeAntennaData failed with status %d\n",status);
  }
  status = writeSourceData(fptr, data);
  if (status !=0) {
    fprintf(stderr,"ERROR: writeS failed with status %d\n",status);
  }
...
int writeSourceData(fitsfile *fptr, uvdata *data) {
    ...
    int tfields   = 19;       /* table will have 3 columns */
    long nrows    = 1;       /* table will have 6 rows    */


    char extname[] = "AIPS SU";           /* extension name */
    ...
    char *ttype[] = {"ID. NO.","SOURCE","QUAL","CALCODE","IFLUX","QFLUX","UFLUX","VFLUX","FREQOFF","BANDWIDTH","RAEPO"   ,"DECEPO" ,"EPOCH","RAAPP"  ,"DECAPP" ,"LSRVEL","RESTFREQ","PMRA"   ,"PMDEC"};
    char *tform[] = {"1J"    ,"16A"   ,"1J"  ,"4A"     ,"1E"   ,"1E"   ,"1E"   ,"1E"   ,"1D"     ,"1D"       ,"1D"      ,"1D"     ,"1D"   ,"1D"     ,"1D"     ,"1D"    ,"1D"      ,"1D"     ,"1D"};
    char *tunit[] = {NULL    ,NULL    ,NULL  ,NULL     ,"JY"   ,"JY"   ,"JY"   ,"JY"   ,"HZ"     ,"HZ"       ,"DEGREES" ,"DEGREES","YEARS","DEGREES","DEGREES","M/SEC" ,"HZ"      ,"DEG/DAY","DEG/DAY"};


    long ID_NO[]={1};
    char *SOURCE[]={"sim.csrh_0"};
    long QUAL[]={0};
    char *CALCODE[]={"OBJ"};
    float IFLUX[]={0.0};
    float QFLUX[]={0.0};
    ...
    if ( fits_movabs_hdu(fptr, 3, &hdutype, &status) ) /* move to 4nd HDU */
         printf("the error");
    ...
    fits_write_col(fptr, TLONG  , 1 , firstrow, firstelem, nrows, ID_NO    , &status);

    fits_write_col(fptr, TSTRING, 2 , firstrow, firstelem, nrows, SOURCE   , &status);
    fits_write_col(fptr, TLONG  , 3 , firstrow, firstelem, nrows, QUAL     , &status);
    fits_write_col(fptr, TSTRING, 4 , firstrow, firstelem, nrows, CALCODE  , &status);
    fits_write_col(fptr, TFLOAT , 5 , firstrow, firstelem, nrows, IFLUX    , &status);
    fits_write_col(fptr, TFLOAT , 6 , firstrow, firstelem, nrows, QFLUX    , &status);
    fits_write_col(fptr, TFLOAT , 7 , firstrow, firstelem, nrows, UFLUX    , &status);
    fits_write_col(fptr, TFLOAT , 8 , firstrow, firstelem, nrows, VFLUX    , &status);
    fits_write_col(fptr, TDOUBLE, 9 , firstrow, firstelem, nrows, FREQOFF  , &status);
    fits_write_col(fptr, TDOUBLE, 10, firstrow, firstelem, nrows, BANDWIDTH, &status);

    fits_write_col(fptr, TDOUBLE, 11, firstrow, firstelem, nrows, &(data->source->ra) , &status);
    fits_write_col(fptr, TDOUBLE, 12, firstrow, firstelem, nrows, &(data->source->dec), &status);
    fits_write_col(fptr, TDOUBLE, 13, firstrow, firstelem, nrows, EPOCH    , &status);
    fits_write_col(fptr, TDOUBLE, 14, firstrow, firstelem, nrows, &(data->source->ra) , &status);
    fits_write_col(fptr, TDOUBLE, 15, firstrow, firstelem, nrows, &(data->source->dec), &status);
    fits_write_col(fptr, TDOUBLE, 16, firstrow, firstelem, nrows, LSRVEL   , &status);
    fits_write_col(fptr, TDOUBLE, 17, firstrow, firstelem, nrows, &(data->source->restfreq) , &status);
    fits_write_col(fptr, TDOUBLE, 18, firstrow, firstelem, nrows, PMRA     , &status);
    fits_write_col(fptr, TDOUBLE, 19, firstrow, firstelem, nrows, PMDEC    , &status);




    long extver=1.,no_if=1.,freqid=1.;
    fits_update_key(fptr, TLONG, "EXTVER", &extver,"", &status);
    fits_update_key(fptr, TLONG, "NO_IF", &no_if,"", &status);
    fits_update_key(fptr, TLONG, "FREQID", &freqid,"", &status);
...
int writeFQbintable (fitsfile *fptr)
{

    int status, hdutype;
    long firstrow, firstelem;

    int tfields   = 5;       /* table will have 3 columns */
    long nrows    = 1;       /* table will have 6 rows    */


    char extname[] = "AIPS FQ";           /* extension name */
    ...
    char *tunit[] = {NULL    ,"HZ"     ,"HZ"      ,"HZ"             ,NULL};


    long FRQSEL[]={1};
    double IF_FREQ[]={-8.6375e08};
    float CH_WIDTH[]={2.5e07};
    float TOTAL_BANDWIDTH[]={2.5e07};
    long SIDEBAND[]={1};
    ...
    fits_write_col(fptr, TLONG, 1, firstrow, firstelem, nrows, FRQSEL, &status);
    fits_write_col(fptr, TDOUBLE,2, firstrow, firstelem, nrows, IF_FREQ,&status);
    fits_write_col(fptr, TFLOAT, 3, firstrow, firstelem, nrows, CH_WIDTH, &status);
    fits_write_col(fptr, TFLOAT, 4, firstrow, firstelem, nrows, TOTAL_BANDWIDTH, &status);
    fits_write_col(fptr, TLONG, 5, firstrow, firstelem, nrows, SIDEBAND, &status);


    long extver=1.,no_if=1.;
    if ( fits_update_key(fptr, TLONG, "EXTVER", &extver,"", &status) )
         printf("the error");
    if ( fits_update_key(fptr, TLONG, "NO_IF", &no_if,"", &status) )
         printf("the error");
...
void Locxyz2ITRF(double latitude, double longitude, double height, double *locx, double *locy, double *locz) {
    double llocx = *locx;
    double llocy = *locy;
    double llocz = *locz+height;

    double phi = latitude * PI / 180.;
    double lmbda = longitude * PI / 180.;
    double sphi = sin(phi);
    double a = 6378137.0;
    double b = 6356752.3142;
    double ae = acos(b / a);
    double N = a / sqrt(1.0 - (sin(ae) * sphi) * (sin(ae) * sphi));

    double Nploczcphimlocysphi = (N + llocz) * cos(phi) - llocy * sphi;

    double clmb = cos(lmbda);
    double slmb = sin(lmbda);
    //  cout << setprecision(15) << "t:" << Nploczcphimlocysphi << " " << clmb << " " << slmb << " " <<  llocx << " " << llocy << " " << llocz <<endl ;

    *locx = Nploczcphimlocysphi * clmb - llocx * slmb;
    *locy = Nploczcphimlocysphi * slmb + llocx * clmb;
    *locz = (N * (b / a) * (b / a) + llocz) * sphi + llocy * cos(phi);
}
